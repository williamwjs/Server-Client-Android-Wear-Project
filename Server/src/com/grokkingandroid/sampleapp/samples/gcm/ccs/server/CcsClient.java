...
public class CcsClient {
    ...
    public BlueBusTrackerServer myserver;
    final static String projectId = "1029481912999";
    final static String password = "AIzaSyDL2PGPcxFuXuOl7tNtnT4JMsZE3X4Czc4";
    final static String toRegId = "/topics/global";
    static CcsClient ccsClient;
    ...
    public void connect() throws XMPPException {
        ...
        connection.addPacketListener(new PacketListener() {
            ...
            public void processPacket(Packet packet) {
                ...
                System.out.println("receive msg :" + json);
                if (myserver.status == 0) {  //receive request
                    String[] msgArr1 = json.split("\\|");
                    if(!"GPS".equals(msgArr1[1]))
                        return;
                    myserver.status = 1;
                    String sendMsg1 = myserver.findChoices(msgArr1[2], msgArr1[3], Integer.parseInt(msgArr1[4]), Integer.parseInt(msgArr1[5]));
                    String messageId = ccsClient.getRandomMessageId();
                    Map<String, String> payload = new HashMap<String, String>();
                    payload.put("message", sendMsg1);
                    String collapseKey = "sample";
                    Long timeToLive = 10000L;
                    Boolean delayWhileIdle = true;
                    ccsClient.send(ccsClient.createJsonMessage(toRegId, messageId, payload, collapseKey,
                            timeToLive, delayWhileIdle));
                    System.out.println("send msg :" + sendMsg1);
                } else if (myserver.status == 1) { //receive choice
                    String[] msgArr2 = json.split("\\|");
                    if(!"BUS".equals(msgArr2[1]))
                        return;
                    myserver.status = 2;
                    int target = Integer.parseInt(msgArr2[2]);
                    //track
                    int stopId = myserver.findDepartureStop(myserver.choices.get(target).lat, myserver.choices.get(target).lon);
                    int routeId = myserver.routeMap.get(myserver.choices.get(target).busName);
                    List<Integer> arrival_time = myserver.getArrivalTime(stopId, routeId);

                    System.out.println("Start tracking bus");
                    while (!myserver.toNotify(arrival_time, myserver.choices.get(target).walking_time, myserver.choices.get(target).duration_time)) {
                        try {
                            Thread.sleep(30000);
                            arrival_time = myserver.getArrivalTime(stopId, routeId);
                        } catch (Exception e) {
                            System.out.println("sleep error");
                        }
                    }

                    System.out.println("Locate the bus, wait to notify the user");
                    long bArrivalTime = ccsClient.myserver.busArrivalTime;
                    Date date = new Date(bArrivalTime * 1000);
                    System.out.println("date : " + date);
                    System.out.println("bArrivalTime : " + bArrivalTime);
                    String sendMsg2;
                    String hour = "" + date.getHours();
                    String mins = "" + date.getMinutes();
                    if (hour.length() < 2) hour = "0" + hour;
                    if (mins.length() < 2) mins = "0" + mins;
                    if (myserver.busArrivalTime != 0)
                        sendMsg2 = (++myserver.mId) + "|Wake|" + myserver.choices.get(target).lat + "," + myserver.choices.get(target).lon
                            + "|" + hour + ":" + mins +"|" + myserver.busNameMap.get(myserver.choices.get(target).busName) + "|";
                    else
                        sendMsg2 = (++myserver.mId) + "|Unable|";

                    String messageId = ccsClient.getRandomMessageId();
                    Map<String, String> payload = new HashMap<String, String>();
                    payload.put("message", sendMsg2);
                    String collapseKey = "sample";
                    Long timeToLive = 10000L;
                    Boolean delayWhileIdle = true;
                    ccsClient.send(ccsClient.createJsonMessage(toRegId, messageId, payload, collapseKey,
                            timeToLive, delayWhileIdle));
                    System.out.println("send msg success :" + sendMsg2);
                    //change back initial condition;
                    myserver.busArrivalTime = 0;
                    myserver.hasBus = false;
                    myserver.status = 0;
                }
            }
    ...
    public static void main(String[] args) {
        ccsClient = CcsClient.prepareClient(projectId, password, true);
        ccsClient.myserver = new BlueBusTrackerServer();
